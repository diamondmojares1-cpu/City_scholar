// UniversityOverview.jsx
import React, { useEffect, useState, useMemo } from "react";
import { collection, getDocs } from "firebase/firestore";
import { db } from "../firebase/firebaseConfig";
import Sidebar from "../components/SidebarSuper.jsx";
import { MapContainer, TileLayer, CircleMarker, Tooltip } from "react-leaflet";
import "leaflet/dist/leaflet.css";
import "../css/UniversityOverview.css";

// ── Known Dagupan universities with coordinates ───────────────
const DAGUPAN_UNIVERSITIES = [
  { name: "Lyceum-Northwestern University (LNU)", lat: 16.0432, lng: 120.3356, color: "#8B0000", shortName: "LNU" },
  { name: "Universidad de Dagupan",               lat: 16.0435, lng: 120.3340, color: "#1565C0", shortName: "UDD" },
  { name: "PHINMA University of Pangasinan",      lat: 16.0450, lng: 120.3370, color: "#1B5E20", shortName: "UPang" },
  { name: "University of Luzon",                  lat: 16.0448, lng: 120.3348, color: "#E65100", shortName: "UL" },
  { name: "Pangasinan Merchant Marine Academy",   lat: 16.0398, lng: 120.3420, color: "#0D47A1", shortName: "PMMA" },
  { name: "Dagupan Colleges Foundation",          lat: 16.0415, lng: 120.3330, color: "#6A1B9A", shortName: "DCF" },
  { name: "STI College Dagupan",                  lat: 16.0460, lng: 120.3395, color: "#B71C1C", shortName: "STI" },
  { name: "Mary Help of Christians College Seminary", lat: 16.0380, lng: 120.3310, color: "#1A237E", shortName: "MHCCS" },
  { name: "Kingfisher School of Business & Finance",  lat: 16.0470, lng: 120.3360, color: "#004D40", shortName: "KSBF" },
  { name: "PIMSAT Colleges",                      lat: 16.0425, lng: 120.3380, color: "#F57F17", shortName: "PIMSAT" },
  { name: "Asiacareer College",                   lat: 16.0440, lng: 120.3315, color: "#00695C", shortName: "ACC" },
  { name: "Graystone Academy",                    lat: 16.0405, lng: 120.3405, color: "#37474F", shortName: "GRA" },
];
...
// ── Bar Chart ─────────────────────────────────────────────────
// Receives only universities with count > 0, sorted by count desc
function BarChart({ data }) {
  ...
  return (
    <div className="uov-bar-chart">
      {data.map((d) => {
        const barHeight = Math.max(4, (d.count / max) * 150);
        return (
          <div key={d.name} className="uov-bar-column" title={`${d.name}: ${d.count}`}>
            ...
            <div
              className="uov-bar"
              style={{ height: `${barHeight}px`, background: d.color }}
            />
...
export default function UniversityOverview() {
  ...
  // Only universities with data, sorted highest first
  ...
  // Table also shows only universities with scholars
  ...
  return (
    <div className="uov-root">
      <Sidebar activePage="university" role="admin" />
      ...
      <main className="uov-main">
        ...
        <div className="uov-body">
          ...
          <div className="uov-right">
            ...
            {/* Bar Chart — only universities with data */}
            ...
            {/* University List — only universities with data */}
