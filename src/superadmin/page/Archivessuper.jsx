    ...
    <div className="arc-confirm-overlay" onClick={onCancel}>
      <div className="arc-confirm-box" onClick={e => e.stopPropagation()}>
        ...
        <div className="arc-confirm-btns">
          <button className="arc-confirm-no"  onClick={onCancel}>No, Cancel</button>
          <button className="arc-confirm-yes" onClick={onConfirm}>Yes, Unarchive</button>
        </div>
      </div>
    </div>
  );
...
function ArchivesSuper() {
  ...
  return (
    <div className="arc-container">
      <SidebarSuper activePage="archives" />
                  ...
                  paginated.map((a, idx) => (
                    <tr key={a.id} className={idx % 2 === 1 ? "arc-tr-alt" : ""}>
                      ...
                      <td className="arc-td-school">
                        {a.schoolName.length > 20 ? a.schoolName.slice(0, 20) + "…" : a.schoolName}
                      </td>
          ...
          {!loading && filtered.length > 0 && (
            <div className="arc-footer">
              ...
              <div className="arc-pagination">
                <button className="arc-pg-prev" onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1}>
                  &lt; Previous
                </button>
                {pageNumbers.map(p => (
                  <button key={p} className={"arc-pg-num" + (p === currentPage ? " arc-pg-active" : "")} onClick={() => goToPage(p)}>
                    {p}
                  </button>
                ))}
                <button className="arc-pg-next" onClick={() => goToPage(currentPage + 1)} disabled={currentPage === totalPages}>
                  Next &gt;
                </button>
              </div>
            </div>
